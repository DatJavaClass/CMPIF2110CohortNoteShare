...
weaves them into a single, connected lesson. Instead of eleven short notes
...
it appears, since the readings and lectures tend to use terms without
stopping to define them. Definitions that go beyond what the lecture or the
reading said are marked "(added definition)". The examples are the notes'
own, built to teach the same lesson the course material teaches. The facts,
terms, dates, and product names stay faithful to the source.
...
2. Two separate checkers read it independently and hunt for errors. One checks
   ...
   runs every scan and traces every number to confirm it is right.
...
| 4 (Part 1) | High (~95%) | Six notes through DocTrio (writer, editor, fact checker, each head a fresh agent), then the lesson through a full Diamond. Every query was run before any writer started, against one small SQLite database the notes own (a bike shop's six tables), and rerun by every fact checker and by both auditors: all 26 statements in the lesson reproduce their printed tables cell for cell, the planted wrong line total surfaces in exactly one row, the sale totals against the line totals differ by exactly the four dollars the lesson says, and the two error examples fail where the book says they fail. Every book query label, result value, figure and page number was checked against the page images (the text layer of the PDFs scrambles the queries), and every dialect claim (the plus sign, the two bars, CONCAT, RTRIM, the two Access boxes, MySQL 8.0's permissive results) matched the source. The execution auditor caught that the database seed had padded the buyer names one character short of the fourteen the lesson states, a defect in the checking harness rather than the lesson, fixed and rerun; the fidelity auditor found no error and raised five framing points, three of which the meta auditor overturned because applying them would have restored the book's own loose wording or marked a bridge the sibling notes already support; the meta auditor caught what both missed, the house credit footer every other lesson in this folder carries, and a summary of the book's grouped ORDER BY query that had quietly swapped in the lesson's own thresholds, and returned five edits, all applied and re-verified |
